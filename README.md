...
The demo exam is not DİM material. The separate 79-question draft dataset is transcribed from the official DİM explanation PDF and remains labelled incomplete because listening questions 1–6 need the official audio. Active attempts and results are currently stored only on the current device. Handwritten images, when uploaded by a signed-in user, are stored in Supabase's private bucket; syncing the attempt/result rows themselves is not implemented yet.
...
4. The optional AI explanation route needs `SUPABASE_SECRET_KEY` and `GEMINI_API_KEY` in Vercel. Choose **Secret** for both and never add `NEXT_PUBLIC_` to either name. The older `SUPABASE_SERVICE_ROLE_KEY` also works. The local import uses the secret key from `.env.local`; it is not required in Vercel solely for the import.
5. In Supabase **Authentication → URL Configuration**, set **Site URL** to the production Vercel address, such as `https://sinaqai.vercel.app`, and add that address to **Redirect URLs**. Keep `http://localhost:3000/**` there too if local sign-in is needed.
6. In Vercel **Deployments**, wait for the latest production deployment to show **Ready**. Open the exam catalog and the 79-question incomplete draft route shown above, then complete a test submission.
